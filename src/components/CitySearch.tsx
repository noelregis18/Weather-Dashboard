  ...
  return <div className="relative w-full max-w-md mx-auto">
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-2">
        <div className="relative flex-grow">
          <Input ref={inputRef} type="text" placeholder="Search for a city..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pr-10 w-full dark:bg-gray-800 dark:text-white dark:border-gray-700" onFocus={() => setShowSuggestions(suggestions.length > 0)} />
          <Search className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 dark:text-gray-500 h-4 w-4" />
          
          {showSuggestions && <div ref={suggestionsRef} className="absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 rounded-md shadow-lg max-h-60 overflow-auto">
              {suggestions.map(city => <div key={city} className="px-4 py-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 dark:text-white" onClick={() => handleSuggestionClick(city)}>
                  {city}
                </div>)}
            </div>}
        </div>
        ...
        <div className="flex gap-2">
          <Button type="submit" variant="default" className="bg-weather-blue hover:bg-weather-blue-dark w-full sm:w-auto">
            Search
          </Button>
